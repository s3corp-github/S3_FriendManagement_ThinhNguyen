...
func TestFriendRepo_GetUserIDsByEmailsWithNoBlocked(t *testing.T) {
	testCases := []struct {
		name           string
		emailsInput    []string
		requestorID    int
		...
		{
			name:           "Get failed with error",
			emailsInput:    []string{"abc@xyz.com"},
			requestorID:    1,
			expectedResult: nil,
			expectedErr:    errors.New("pq: password authentication failed for user \"postgrespassword=000000\""),
			preparePath:    "",
		...
		{
			name:           "Get success with no block",
			emailsInput:    []string{"xyz@abc.com"},
			requestorID:    1,
			expectedResult: []int{2},
			expectedErr:    nil,
			preparePath:    "../testhelpers/preparedata/datafortest",
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			// When
			result, err := friendRepo.GetUserIDsByEmailsWithNoBlocked(testCase.emailsInput, testCase.requestorID)
